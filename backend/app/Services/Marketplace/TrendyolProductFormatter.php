...
class TrendyolProductFormatter
{
    ...
    {
        $images = $product->images->map(fn ($img) => ['url' => $img->image_url])->toArray();
    ...
    protected function formatVariantItem(
        ...
        $images = $variantImages->isNotEmpty()
            ? $variantImages->map(fn ($img) => ['url' => $img->image_url])->toArray()
            : $product->images->map(fn ($img) => ['url' => $img->image_url])->toArray();
