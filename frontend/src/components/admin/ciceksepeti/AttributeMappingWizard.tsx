...
export default function AttributeMappingWizard({
  ...
  const prepareMutation = useMutation({
    mutationFn: async (productIds: number[]) => {
      const { data } = await api.post<HBPrepareSendResponse>(
        '/admin/ciceksepeti/products/prepare-send',
    ...
    mutationFn: async (payload: {
      ...
      for (const group of payload.groups) {
        if (group.attributes.length > 0) {
          promises.push(
            api.post('/admin/ciceksepeti/products/save-category-attributes', {
      ...
      for (const pp of payload.perProduct) {
        if (pp.attributes.length > 0) {
          promises.push(
            api.post('/admin/ciceksepeti/products/save-attributes', {
  ...
  const sendMutation = useMutation({
    mutationFn: async (productIds: number[]) => {
      const { data } = await api.post<SendBatchResponse>('/admin/ciceksepeti/products/send', {
        product_ids: productIds,
      ...
      pollingRef.current = setInterval(async () => {
        try {
          const { data } = await api.get<SendProgressResponse>(
            `/admin/ciceksepeti/products/send-progress/${result.batchId}`
          );
  ...
  return (
    <Dialog isOpen={isOpen} onClose={isSending ? () => {} : handleClose} className="max-w-5xl">
      ...
      <DialogHeader>
        <DialogTitle>Çiçeksepeti Ürün Gönderim Sihirbazı</DialogTitle>
        <div className="mt-4">
          <WizardStepper
            steps={WIZARD_STEPS}
            currentStep={step}
                    ...
                    <div className="flex flex-col items-center text-center">
                      ...
                      <p className="mt-1 text-sm text-secondary-500">
                        {sendingState.finished
                          ...
                          : 'Lütfen bekleyin, ürünler Çiçeksepeti\'ya gönderiliyor.'}
                      </p>
                    </div>
        ...
        {!sendingState && (
          <>
            ...
            <Button onClick={handleNext} disabled={isLoading || !prepareData}>
              ...
              {step === 3 && (sendMutation.isPending ? 'Gönderiliyor...' : "Çiçeksepeti'ya Gönder")}
            </Button>
          </>
        )}
      </DialogFooter>
