...
export default function AttributeMappingWizard({
  ...
  const [prepareData, setPrepareData] = useState<PrepareSendResponse | null>(null);
  ...
  const prepareMutation = useMutation({
    mutationFn: async (productIds: number[]) => {
      const { data } = await api.post<PrepareSendResponse>(
        '/admin/trendyol/products/prepare-send',
    ...
    onSuccess: (data) => {
      ...
      // Initialize group states with existing values
      ...
      data.categoryGroups.forEach((group) => {
        ...
        for (const attr of group.categoryAttributes) {
          ...
          // For autoMatched slicer/varianter attributes: populate ALL products'
          // values so they get saved to product_attributes during the save step.
          // This is critical because existingAttributes may include name-detected
          // or variant-detected values that aren't in product_attributes yet.
          if (attr.autoMatched) {
          ...
          if (colorAttrNames.has(attrNameLower)) {
            // Renk/Web Color: her ürünün kendi değerini al, kategori seviyesine koyma
            for (const product of group.products) {
              const existing = product.existingAttributes.find(
                (ea) => ea.attribute_name.toLowerCase() === attrNameLower,
              );
    ...
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || 'Hazirlik verisi alinamadi');
    },
  });
    ...
    mutationFn: async (payload: {
      ...
      for (const group of payload.groups) {
        if (group.attributes.length > 0) {
          promises.push(
            api.post('/admin/trendyol/products/save-category-attributes', {
      ...
      for (const pp of payload.perProduct) {
        if (pp.attributes.length > 0) {
          promises.push(
            api.post('/admin/trendyol/products/save-attributes', {
  ...
  const sendMutation = useMutation({
    mutationFn: async ({ productIds, overrides }: { productIds: number[]; overrides?: Record<number, number> | null }) => {
      const payload: Record<string, unknown> = {
        product_ids: productIds,
        min_stock: minStockThreshold,
      };
      if (overrides && Object.keys(overrides).length > 0) {
        payload.price_overrides = overrides;
      }
      const { data } = await api.post<SendBatchResponse>('/admin/trendyol/products/send', payload);
      return data;
    },
    onSuccess: (result) => {
      setSendingState({
      ...
      pollingRef.current = setInterval(async () => {
        try {
          const { data } = await api.get<SendProgressResponse>(
            `/admin/trendyol/products/send-progress/${result.batchId}`
          );
          ...
          if (data.finished || data.cancelled) {
            stopPolling();
            if (data.cancelled) {
              toast.error('Gonderim iptal edildi.');
          ...
          // Polling hatasi — sessizce devam et
        }
      }, 3000);
    },
    onError: (error: any) => {
  ...
  // Track whether we've initiated the prepare call for the current open
  ...
  const handleClose = () => {
    if (isSending) return; // Gonderim sirasinda kapatma
    stopPolling();
    setHasFetched(false);
    setPrepareData(null);
    setGroupStates({});
  ...
  const validateStep2 = (): boolean => {
    ...
    for (const group of prepareData.categoryGroups) {
      ...
      for (const attr of group.categoryAttributes) {
        ...
        if (attr.autoMatched) {
          // For autoMatched attrs, validate products that don't have a resolved value
          // Category-level value covers all unmatched products
  ...
  const handleNext = async () => {
    ...
    if (step === 2) {
      ...
      for (const group of prepareData.categoryGroups) {
        ...
        // Per-product attributes (kategori seviyesinde değer varsa o attr'ı atla)
        for (const [productIdStr, values] of Object.entries(state.perProductValues)) {
          ...
          for (const [name, value] of Object.entries(values as AttributeFormValues)) {
            if (value && !state.categoryValues[name]) {
    ...
    if (step === 3) {
      ...
      sendMutation.mutate({ productIds: ids, overrides: priceOverrides });
    }
  };
  ...
  return (
    <Dialog isOpen={isOpen} onClose={isSending ? () => {} : handleClose} className="max-w-5xl">
      ...
      <DialogHeader>
        <DialogTitle>Trendyol Ürün Gönderim Sihirbazı</DialogTitle>
        <div className="mt-4">
          <WizardStepper
            steps={WIZARD_STEPS}
            currentStep={step}
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-lg border border-secondary-200 p-3">
                  <div className="flex items-center gap-3">
                    <label htmlFor="min-stock" className="text-sm font-medium text-secondary-700 whitespace-nowrap">
                      Kritik Stok
                    </label>
                    <input
                      id="min-stock"
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline">
                            {group.categoryAttributes.filter((a) => a.required && !a.autoMatched).length} zorunlu özellik
                          </Badge>
                        </div>
                      </div>
                    ))}
                ...
                {sendingState ? (
                  ...
                  <div className="mx-auto max-w-md space-y-5">
                    <div className="flex flex-col items-center text-center">
                      ...
                      <p className="mt-1 text-sm text-secondary-500">
                        {sendingState.finished
                          ...
                          : 'Lütfen bekleyin, ürünler Trendyol\'a gönderiliyor.'}
                      </p>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-secondary-600">Ilerleme</span>
                      ...
                      <p className="text-center text-xs text-secondary-500">
                        {sendingState.processedJobs} / {sendingState.totalBatches} batch tamamlandi
                      </p>
                    </div>
                  ...
                  <>
                    <div className="flex flex-col items-center text-center">
                      ...
                      <p className="mt-1 text-sm text-secondary-500">
                        Asagidaki ozet bilgileri kontrol edip gonderimi baslatin.
                      </p>
                    </div>
                    ...
                    <div className="mx-auto max-w-md space-y-3">
                      ...
                      <div className="flex items-center justify-between rounded-lg border border-secondary-200 p-3">
                        <span className="text-sm text-secondary-600">Kategori sayisi</span>
                        <Badge variant="outline">
                      ...
                      <div className="flex items-center justify-between rounded-lg border border-secondary-200 p-3">
                        <span className="text-sm text-secondary-600">Batch sayisi</span>
                        <Badge variant="outline">
                      ...
                      {unmappedGroups.length > 0 && (
                        <div className="flex items-center justify-between rounded-lg border border-red-200 bg-red-50 p-3">
                          <span className="text-sm text-red-700">Kategori eslesmesi yok (haric)</span>
        ...
        {!sendingState && (
          <>
            ...
            <Button onClick={handleNext} disabled={isLoading || !prepareData}>
              ...
              {step === 1 && 'Ileri'}
              {step === 2 && (saveCategoryAttrsMutation.isPending ? 'Kaydediliyor...' : 'Kaydet & Ileri')}
              {step === 3 && (sendMutation.isPending ? 'Gönderiliyor...' : "Trendyol'a Gönder")}
            </Button>
          </>
        )}
      </DialogFooter>
