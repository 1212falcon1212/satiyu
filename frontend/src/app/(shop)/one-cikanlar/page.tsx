...
export const metadata: Metadata = {
  title: 'Öne Çıkan Ürünler - Giyim Mağazası',
  description: 'Giyim Mağazası\'nın öne çıkan moda ve giyim ürünleri. Seçilmiş en iyi ürünler uygun fiyatlarla.',
};
...
async function fetchFeaturedProducts(params: Record<string, string | undefined>) {
  const qs = new URLSearchParams();
  qs.set('is_featured', '1');
  qs.set('per_page', '20');
...
export default async function FeaturedPage({ searchParams }: PageProps) {
  ...
  const [results, brands, categories] = await Promise.all([
    fetchFeaturedProducts(sp).catch(() => null),
    getBrands(),
    getCategories(),
  ]);
  ...
  return (
    ...
    <div className="container-main py-6">
      ...
      <nav className="mb-4 flex items-center gap-1 text-sm text-secondary-400">
        ...
        <span className="text-secondary-700">Öne Çıkan Ürünler</span>
      </nav>
        ...
        <div className="min-w-0 flex-1">
          ...
          {results.data.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <Star className="h-12 w-12 text-secondary-300" />
              <p className="mt-4 text-lg font-medium text-secondary-900">Öne çıkan ürün bulunamadı</p>
